...
class DataloadManager():
    def __init__(
        ...
        if self.args.dataset in ['hateful_memes', 'crisis_mmd']:
        ...
        if self.args.dataset in ['uci_har', 'extrasensory', "ku_har"]:
    ...
    def get_client_ids(
        ...
        elif self.args.dataset in ["hateful_memes", "crisis_mmd"]:
            alpha_str = str(self.args.alpha).replace('.', '')
        ...
        elif self.args.dataset in ["uci_har"]:
            alpha_str = str(self.args.alpha).replace('.', '')
        ...
        elif self.args.dataset in ["extrasensory", "extrasensory_watch", "ku_har"]:
            data_path = self.acc_feat_path.joinpath(
                f'fold{fold_idx}'
            )
    ...
    def load_gyro_feat(
        ...
        elif self.args.dataset in ["extrasensory", "ku_har"]:
            data_path = self.gyro_feat_path.joinpath(
                f'fold{fold_idx}', 
                f'{client_id}.pkl'
            )
    ...
    def load_ecg_feat(
        ...
        num_samples1 = len(i_to_avf_data_dict)
        num_samples2 = len(v1_to_v6_data_dict)   
            
        split_idx = int(num_samples1 * 0.8)
        train_i_to_avf_data_dict = i_to_avf_data_dict[:split_idx]  
        test_i_to_avf_data_dict = i_to_avf_data_dict[split_idx:]
        
        split_idx = int(num_samples2 * 0.8)
        train_v1_to_v6_data_dict = v1_to_v6_data_dict[:split_idx]  
        test_v1_to_v6_data_dict = v1_to_v6_data_dict[split_idx:]
        return v1_to_v6_data_dict, test_i_to_avf_data_dict, train_v1_to_v6_data_dict, test_v1_to_v6_data_dict
    ...
    def load_sim_dict(
        ...
        if self.args.dataset in ["ucf101", "crema_d", 'ku_har']:
        ...
        elif self.args.dataset in ["mit10", "mit51", "meld", "uci_har", "ptb-xl", "crisis_mmd", "hateful_memes"]:
            data_path = Path(self.args.data_dir).joinpath(
                'simulation_feature',
                self.args.dataset,
                f'{self.setting_str}.{ext}'
